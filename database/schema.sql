...
CREATE UNIQUE INDEX idx_blog_subscribers_email ON blog_subscribers (LOWER(email));
CREATE INDEX idx_blog_subscribers_status ON blog_subscribers (status);
CREATE INDEX idx_blog_subscribers_last_digest_sent ON blog_subscribers (last_digest_sent_at);

CREATE TABLE daily_email_logs (
  id              UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_daily_email_logs_date ON daily_email_logs (dispatch_date);
